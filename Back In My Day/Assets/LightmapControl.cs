...
public class LightmapControl : MonoBehaviour
{
    ...
    public void Load()
    {
        ...
        for (int i = 0; i < newLightmaps.Length; i++)
        {
            ...
            newLightmaps[i].lightmapColor = Resources.Load<Texture2D>("Lightmaps/" + m_resourceFolder + "/" + lightingScenariosData.lightmaps[i].name);

            if (lightingScenariosData.lightmapsMode != LightmapsMode.NonDirectional)
            {
                newLightmaps[i].lightmapDir = Resources.Load<Texture2D>("Lightmaps/" + m_resourceFolder + "/" + lightingScenariosData.lightmapsDir[i].name);
                if (lightingScenariosData.lightmapsShadow[i] != null)
                { // If the textuer existed and was set in the data file.
                    newLightmaps[i].shadowMask = Resources.Load<Texture2D>("Lightmaps/" + m_resourceFolder + "/" + lightingScenariosData.lightmapsShadow[i].name);
                }
            }
        }
    ...
    public static LightmapControl instance;
    private void Awake()
    {
        if (instance != null)
        { // Singleton pattern.
